...
export default function JobPostingBuilder() {
  ...
  const canReviewJd = !!(user?.permissions?.can_review_jd || user?.permissions?.is_admin)
  const isReviewMode = canReviewJd && requestData?.status === 'Sent for Approval'
  const readOnly = !canBuildJd || requestData?.status === 'Approved' || requestData?.status === 'Posted'
